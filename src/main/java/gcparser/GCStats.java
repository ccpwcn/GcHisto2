...
public class GCStats {
    ...
    GCStats(EnumMap<GCMetric, Boolean> enabledMap, int cpuCount,
            boolean inputHasTimeZero, int fileTimestampGap,
            int fileTimestampGapMin) {
        _enabled_map = enabledMap;
        _stats_map = new EnumMap<GCMetric, Stats>(GCMetric.class);
        for (GCMetric metric : GCMetric.values()) {
            _stats_map.put(metric, new Stats());
        }
        _cpu_count = cpuCount;
        _window_data = new TimingWindowData(cpuCount);

        _input_has_time_zero = inputHasTimeZero;
        if (!_input_has_time_zero) {
            _timestamp_beg = -1.0;
        }

        _file_timestamp_gap = fileTimestampGap;
        _file_timestamp_gap_min = fileTimestampGapMin;
